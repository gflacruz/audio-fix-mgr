...
import { getRepair, updateRepair, deleteRepair, getRepairNotes, addRepairNote, getTechnicians, getParts, addRepairPart, removeRepairPart, addCustomRepairPart, uploadRepairPhoto, deleteRepairPhoto, sendEstimateEmail, sendPickupEmail, sendEstimateText, sendPickupText, getEstimates, updateEstimate } from '@/lib/api';
...
const RepairDetail = () => {
  ...
  const [emailModal, setEmailModal] = useState({
    ...
    step: 'idle', // 'no-estimates', 'select-estimate', 'confirm', 'sending', 'success', 'error'
    error: null,
  ...
  const executeEstimateAction = async () => {
    ...
    try {
      ...
      if (type === 'approve') {
        ...
        // 2. Update Repair Status and Labor Cost
        await updateRepair(id, {
          status: 'repairing',
          laborCost: estimate.laborCost
        });
        
        // 3. Add Custom Part for Parts Cost if > 0
        ...
        // 4. Deny all other non-approved estimates
  ...
  const handlePendingEstimate = async (e, estimate) => {
    e.stopPropagation();
    try {
      await updateEstimate(estimate.id, {
        status: 'pending'
